...
def search(field, value):
    ...
    if order == "asc":
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s where p.id in (select pet from tags_pets tp where tp.tag=(select id from tag where name='{value}')) and p.species = s.id order by p.{oby}")
    else:
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s where p.id in (select pet from tags_pets tp where tp.tag=(select id from tag where name='{value}')) and p.species = s.id order by p.{oby}desc")    
    pets = cursor.fetchall()
    return render_template('search.html', pets = pets, field = "tag", value = value, order = "desc" if order=="asc" else "asc")
